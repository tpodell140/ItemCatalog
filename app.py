...
def gconnect():
    ...
    if result['issued_to'] != CLIENT_ID:
        response = make_response(json.dumps(
            "Token's client ID doesn't match given client ID."), 401)
        response.headers['Content-Type'] = 'application/json'
        return response
    ...
    output += ' " style = "width: 75px; height: 75px;border-radius: 150px;'
...
def showDealers():
    ...
    return (render_template('dealerships.html',
                            dealerships=dealerships, STATE=state))
...
def displayInventory(dealer_id):
    ...
    return (render_template('displayInventory.html', dealership=dealership,
...
def addCar(dealer_id):
    ...
    if request.method == 'POST':
        ...
        if not image:
            image = 'http://myclassicgarage.com/assets/'
            'image_placeholder_low_res.png'
        newCar = Car(
...
def displayCar(dealer_id, car_id):
    ...
    return (render_template('displayCar.html', dealership=dealership,
                            car=car, STATE=state))


# Edit Information for a specific car
@app.route('/dealerships/<int:dealer_id>/cars/<int:car_id>/edit',
           methods=['GET', 'POST'])
...
@app.route('/dealerships/<int:dealer_id>/cars/<int:car_id>/delete',
           methods=['GET', 'POST'])
